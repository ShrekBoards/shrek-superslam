...
pub(crate) fn construct_array<T>(
    ...
    console: Console,
    array_offset: usize,
    array_count_offset: usize) -> Result<Vec<T>, Error>
where
    T: SerialisedShrekSuperSlamGameObject
{
    let list_offset = console.read_u32(&bin.raw[initial_object_offset + array_offset..initial_object_offset + array_offset + 4])? as usize;
    let list_count = console.read_u32(&bin.raw[initial_object_offset + array_count_offset..initial_object_offset + array_count_offset + 4])? as usize;
...
pub(crate) fn construct_optional_type<T>(
    ...
    initial_object_offset: usize,
    console: Console,
    object_pointer_offset: usize) -> Result<Option<T>, Error>
where
    T: SerialisedShrekSuperSlamGameObject
{
    let object_offset = console.read_u32(&bin.raw[initial_object_offset + object_pointer_offset..initial_object_offset + object_pointer_offset + 4])?;
